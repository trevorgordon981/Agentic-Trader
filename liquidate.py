...
async def cancel_resting_closes(ib, con_ids):
    ...
    that touches a contract we are about to liquidate at market. Without this, the tool's market
    close AND a resting exit-manager SELL can BOTH fill -> the position is over-sold / the short leg
    left naked (the 6/29 double-close family). Matches single-leg orders by conId and combo (BAG)
    orders by any leg's conId. Returns the count cancelled. Never raises (best-effort)."""
...
async def run(confirm, client_id):
    ...
    if _ncx:
        slack(f":scissors: liquidate: cancelled {_ncx} resting exit order(s) before closing.")
    ...
    # LAYER 1b (2026-06-29): journal our closes so the trader's exit manager won't re-close an
    # already-closed position (the double-close that left a +2 long residual).
    try:
        ...
        with open(jpath, "a") as _jf:
            for c, tr in trades:
                ...
                # per-leg markers. `tool` distinguishes exit_reason (liquidated vs closed_by_tool).
                ...
                _jf.write(json.dumps({"ts": _dt.datetime.utcnow().isoformat(),
                    ...
                    "status": _st, "avg_fill_price": _afp, "tool": "liquidate",
                    "client_id": client_id}) + "\n")
