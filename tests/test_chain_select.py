...
def test_pick_chain_prefers_standard_trading_class():
    params = [_p("SMART", "RKLB1", [1, 2]), _p("SMART", "RKLB", [1, 2, 3])]
...
def test_strikes_near_windows_around_spot_drops_stale():
    ...
    assert 174.78 not in near and 179.78 not in near and 184.78 not in near
    assert all(k >= 500 for k in near)

    assert near == [float(x) for x in range(500, 701, 5)]
