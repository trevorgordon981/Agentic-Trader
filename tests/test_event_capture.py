...
def test_emit_appends_event(_store):
    rec = ec.emit("entry_decision", trade_uid="T1", symbol="AAPL", con_id=42,
...
def test_order_ids_flatten_and_dedup(_store):
    rec = ec.emit("order_fill", order_ids=[1, 1, None, 0, "0", 2])
...
def test_blob_hash_roundtrip(_store):
    ...
    rec = ec.emit("entry_decision", trade_uid="T2", context=ctx)
...
def test_blob_dedup_write_once(_store):
    ctx = {"a": 1, "b": [1, 2, 3]}
    r1 = ec.emit("no_trade", context=ctx)
    r2 = ec.emit("no_trade", context=dict(ctx))
    assert r1["context_sha256"] == r2["context_sha256"]
    blobs = os.listdir(os.path.join(str(_store), "blobs"))
    assert len(blobs) == 1


def test_empty_context_no_blob(_store):
    rec = ec.emit("no_trade", context=None)
    assert rec["context_sha256"] is None
    rec2 = ec.emit("no_trade", context={})
    assert rec2["context_sha256"] is None



def test_eightx_dte_pass_fail():
    ok = ec.eightx_dte_check(dte=80, trade_window_days=10)
    assert ok["dte"] == 80 and ok["trade_window_days"] == 10.0
    assert ok["ratio"] == 8.0 and ok["rule_ok"] is True
    bad = ec.eightx_dte_check(dte=20, trade_window_days=10)
    assert bad["ratio"] == 2.0 and bad["rule_ok"] is False


def test_eightx_dte_from_expiry():
    r = ec.eightx_dte_check(expiry="2026-08-20", trade_window_days=5,
                            ref_date="2026-07-21")
    assert r["dte"] == 30 and r["rule_ok"] is False
    r2 = ec.eightx_dte_check(expiry="20260820", ref_date="20260721")
    assert r2["dte"] == 30 and r2["rule_ok"] is None


def test_eightx_dte_unknown_window_records_dte():
    r = ec.eightx_dte_check(dte=45)
...
def test_on_exit_event(_store):
    ...
    out = ec.on_exit(exit_rec, je, 7, mfe=55.0, mae=-8.0, mark_path=mark_path)
...
def test_on_position_mark_lightweight(_store):
    out = ec.on_position_mark(7, symbol="TSLA", pnl_pct=12.5,
    ...
    assert out["context_sha256"] is None
...
def test_attach_label(_store):
    assert ec.attach_label("U9", {"label_type": "process_grade",
...
def test_attach_label_rejects_bad_input(_store):
    assert ec.attach_label("", {"x": 1}) is False
    assert ec.attach_label("U1", "not a dict") is False
...
def test_emit_never_raises_on_unserializable(_store):
    ...
    out = ec.emit("entry_decision", trade_uid="W1", context={"obj": Weird()})
    assert out is not None and out["trade_uid"] == "W1"


def test_hooks_never_raise_on_garbage(_store):

    assert ec.on_decision(None) is None
    assert ec.on_no_trade(123) is None
    assert ec.on_rejected("x") is None
    assert ec.on_exit("x", None, None) is not None or True
    assert ec.on_fill(None) is None
