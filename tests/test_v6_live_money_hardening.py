...
_FUTURE_EXPIRY = (_dt.date.today() + _dt.timedelta(days=60)).strftime("%Y%m%d")
...
def _trade(*, ref="decision-1", status="Submitted", cid=101, total=1, price=2.5):
    return SimpleNamespace(
        ...
        contract=SimpleNamespace(conId=cid, lastTradeDateOrContractMonth=_FUTURE_EXPIRY),
...
def test_working_buy_is_in_budget_and_not_double_counted_after_partial_fill(tmp_path):
    ...
    journal.write_text(json.dumps({
        "contract_id": 101, "debit": 250, "expiry": _FUTURE_EXPIRY,
    ...
    pos = SimpleNamespace(avg_cost=2.5, quantity=1, expiry=_FUTURE_EXPIRY)
...
def test_correlated_index_positions_still_bind_aggregate_budget(tmp_path):
    ...
    for cid, symbol in ((1, "SPY"), (2, "QQQ"), (3, "IWM")):
        rows.append(json.dumps({"contract_id": cid, "debit": 125, "expiry": _FUTURE_EXPIRY,
                                "decision_id": f"d{cid}"}))
        positions[cid] = SimpleNamespace(avg_cost=1.25, quantity=1, expiry=_FUTURE_EXPIRY,
                                          symbol=symbol)
    journal.write_text("\n".join(rows) + "\n")
...
def test_position_view_exposes_window_and_legacy_nulls(tmp_path):
    ...
    pos = SimpleNamespace(con_id=1, quantity=1, avg_cost=2, symbol="SPY", expiry=_FUTURE_EXPIRY)
...
def test_journal_persists_source_bound_hold(tmp_path):
    ...
    trader.runtime_identity = RuntimeIdentity("a" * 40, "b" * 64)
    order = ResolvedOrder("SPY", "C", _FUTURE_EXPIRY, 600, 1, 2.0,
