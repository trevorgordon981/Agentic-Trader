...
def test_format_contains_key_facts_and_text_hint():
    ...
    assert "Expires in 15m" in msg


def test_autonomous_receipt_body_has_no_interactive_copy():
    msg = format_proposal(IDEA, 1010.0, 121.0, 15, approval_required=False)
    assert "SPY" in msg and "Proposed trade" in msg
    assert "to APPROVE or" not in msg
    assert "Expires in" not in msg


def test_decision_notice_distinguishes_request_from_receipt():
    manual = ap.format_decision_notice("facts", "decision-abc", 30, approval_required=True)
    receipt = ap.format_decision_notice("facts", "decision-abc", 30, approval_required=False)
    assert "to APPROVE or" in manual and "approval expires in 30 minutes" in manual
    assert "to APPROVE or" not in receipt and "approval expires" not in receipt
    assert "decision-abc" in manual and "decision-abc" in receipt
...
def test_replies_skip_parent_and_respect_approver():
    msgs = [
        {"ts": "P", "user": "BOT", "text": "Proposed trade ..."},
        {"ts": "1", "user": "STRANGER", "text": "send it"},
        {"ts": "2", "user": "OWNER", "text": "go for it"},
    ]
...
def test_parse_add_tickers():
    ...
    assert parse_add_tickers("add NVDA", cands) == []
