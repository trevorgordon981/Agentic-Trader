...
# ---------------------------------------------------------------------------
# 2026-07-02 CONVICTION-SCALE PIN (audit Finding 1). The strategist emits conviction on a
# 1-10 scale (strategist.py rubric + `max(1, min(10, ...))` clamp; also technical_card.py,
# approval.py "/10", daily_recommend "/10"). risk.py's curve keys, the cap-bypass threshold,
# and ProposedTrade.conviction MUST stay on that SAME 1-10 scale, or the >=6 confident bypass
# becomes unreachable dead code (the old "1-5" mis-doc implied exactly that). These pins fail
# loudly if anyone drifts the scale again.
# ---------------------------------------------------------------------------
def test_conviction_scale_is_1_to_10():
    # the sizing curve is keyed on the strategist's full 1-10 range, no more, no less
...
def test_cap_bypass_threshold_is_reachable_on_strategist_scale():
    # the confident-sizing bypass fires at conviction >= cap_bypass_min_conviction. That threshold
    # must be a value the strategist can actually emit (1..10), else the bypass is dead code.
    thr = RiskLimits().cap_bypass_min_conviction
    assert 1 <= thr <= 10, f"cap_bypass_min_conviction={thr} is off the 1-10 strategist scale"
    # and it must be a key the sizing curve defines (so a bypassed trade has a curve fraction)
    assert thr in DEFAULT_CONVICTION_SIZE_CURVE


def test_strategist_clamps_to_1_to_10():
    # producer-side pin: the strategist parser clamps conviction into 1..10, matching risk.py's scale
...
def test_confident_bypass_is_live_at_threshold_conviction():
    # behavioral proof the >=6 bypass is NOT dead code: a conviction == threshold trade under a
    # confident, up-sized curve exceeds the 12% soft cap (reaches toward the 25% hard ceiling).
    thr = RiskLimits().cap_bypass_min_conviction
    lim = RiskLimits(confident_full_size=True, allow_any_name=True,
                     conviction_size_curve={c: (0.25 if c >= thr else 0.12) for c in range(1, 11)})
    # $200 on a $1010 pot: rejected under the 12% soft cap (~$121), approved once the bypass lifts
    # the cap to 25% (~$252). This can only pass if conviction==thr actually triggers `confident`.
    d = gate(ProposedTrade("MU", 200.0, False, conviction=thr), available=1010.0, limits=lim)
    assert d.approved, d.reasons
