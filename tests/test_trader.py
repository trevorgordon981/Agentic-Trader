...
async def test_strategist_receives_exact_live_account_snapshot(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=PotSnapshot(5000.0, 4000.0, 4000.0)))
    seen = {}
    ...
    assert "Net liquidation value: $5,000.00" in seen["context"]
    assert "Available funds: $4,000.00" in seen["context"]
...
async def test_research_failure_fallback_keeps_account_snapshot(tmp_path, monkeypatch):
    ...
    context = await t._market_context(
        [], 0.0, net_liq=5000.0, available_funds=4000.0)
    assert "Net liquidation value: $5,000.00" in context
    assert "Available funds: $4,000.00" in context
...
async def test_dry_run_never_executes(tmp_path, monkeypatch):
    ...
    assert posts and posts[0].startswith("[DRY RUN")
...
async def test_trader_defers_model_when_slate_active(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(trader, "propose_intents", lambda *a, **k: [])
...
def test_slate_lock_flag_set_clear_and_staleness(tmp_path, monkeypatch):
    ...
    assert slate_lock.slate_active() is False
    slate_lock.mark_slate_active()
    assert slate_lock.slate_active() is True
