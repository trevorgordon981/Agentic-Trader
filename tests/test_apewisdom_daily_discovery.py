...
class FakeIB:
    ...
    async def reqSecDefOptParamsAsync(self, *_args):
        return [SimpleNamespace(exchange="SMART", tradingClass="MUTX")]
...
async def test_probe_requires_profile_stock_and_options_chain(monkeypatch):
    ...
    assert row["ticker"] == "MUTX" and reason == "eligible"
...
def test_discovery_merge_dedupes_without_mutation():
    broad = [("AAPL", "broad"), ("MUTX", "broad duplicate")]
    ape = [("MUTX", "attention"), ("IBM", "attention")]
    assert dr._merge_discovery_candidates(broad, ape) == [
        ("AAPL", "broad"), ("MUTX", "broad duplicate"), ("IBM", "attention")]
...
def test_main_proposal_and_training_capture_never_receive_attention_context():
    ...
    assert "_res = await _await_slate_model(" in source
...
def test_clarified_source_fields_exclude_24h_and_upvotes():
    context = aw.discovery_context(
        ...
        price_stats={"MUTX": {"ret_20d": 4.56}})
    bound = aw.bind_reviewed_candidates(
        [("MUTX", "ignore me")], [good_row()],
        price_stats={"MUTX": {"ret_20d": 4.56}})
    combined = context + "\n" + bound[0][1]
...
def test_existing_human_add_and_explicit_approval_gates_remain_present():
    ...
    assert "approved = (autonomous_candidate" in source
    assert "or approval.decision_from_reactions(" in source
    assert "or approval.decision_from_replies(" in source
    assert "reactions, approver_ids) == \"approve\"" in source
    assert "replies, approver_ids, ts) == \"approve\"" in source
