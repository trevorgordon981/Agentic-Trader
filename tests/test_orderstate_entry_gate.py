...
async def test_name_closed_in_exit_cycle_not_counted_by_entry_gate(tmp_path, monkeypatch):
    ...
    pre = {1000 + i: _pos(1000 + i, f"SYM{i}") for i in range(8)}
    post = {1000 + i: _pos(1000 + i, f"SYM{i}") for i in range(7)}
    st = {"exited": False}

    async def _get_positions(*, include_short=False):
    ...
    ibc.get_positions.assert_any_await(include_short=True)
...
async def test_stale_book_would_have_blocked_regression_guard(tmp_path, monkeypatch):
    ...
    stub_stage_a(monkeypatch, lambda *a, **k: [SPY_IDEA])
    t, ibc, em = _trader(tmp_path)
    full = {1000 + i: _pos(1000 + i, f"SYM{i}") for i in range(8)}
...
async def test_intracycle_sequential_gating_still_blocks_second_idea(tmp_path, monkeypatch):
    ...
    t, ibc, em = _trader(tmp_path)
    seven = {1000 + i: _pos(1000 + i, f"SYM{i}") for i in range(7)}
...
async def test_fresh_and_stale_book_use_same_path(tmp_path, monkeypatch):
    ...
    stub_stage_a(monkeypatch, lambda *a, **k: [SPY_IDEA])
    t, ibc, em = _trader(tmp_path)
    five = {1000 + i: _pos(1000 + i, f"SYM{i}") for i in range(5)}
