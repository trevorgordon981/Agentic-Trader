...
def test_cli_validates_typed_config_before_marker_checks(monkeypatch):
    from exitmgr import config as config_mod

    marker = Mock()
    monkeypatch.setattr(config_mod, "load_config",
                        Mock(side_effect=RuntimeError("trading.sector_map.CF invalid")))
    monkeypatch.setattr(entry_safety, "entry_markers_clear", marker)
    monkeypatch.setattr(sys, "argv", ["entry_safety", "--config", "bad.yaml"])

    assert entry_safety._cli() == 2
...
def test_live_human_authority_expires_before_callback(monkeypatch):
    authority = entry_safety.EntrySubmissionAuthority.human(
        100.0, ttl_seconds=30)
    called = Mock()
    monkeypatch.setattr(entry_safety.time, "monotonic", lambda: 130.001)
    with pytest.raises(entry_safety.ApprovalExpiredAtSubmit) as exc:
        entry_safety.place_with_live_authority(authority, called)
    assert exc.value.alfred_place_invoked is False
    called.assert_not_called()


def test_live_authority_exact_deadline_and_autonomous_are_allowed(monkeypatch):
    monkeypatch.setattr(entry_safety.time, "monotonic", lambda: 130.0)
    human = entry_safety.EntrySubmissionAuthority.human(100.0, ttl_seconds=30)
    assert entry_safety.place_with_live_authority(human, lambda: "human") == "human"
    monkeypatch.setattr(entry_safety.time, "monotonic", lambda: 999999.0)
    autonomous = entry_safety.EntrySubmissionAuthority.autonomous()
    assert entry_safety.place_with_live_authority(
        autonomous, lambda: "auto") == "auto"


@pytest.mark.parametrize("authority", [
    None,
    entry_safety.EntrySubmissionAuthority("unknown"),
    entry_safety.EntrySubmissionAuthority(
        entry_safety.AUTONOMOUS_WITHIN_RISK_GATES, 123.0),
    entry_safety.EntrySubmissionAuthority(
        entry_safety.HUMAN_SLACK_APPROVAL, None),
])
def test_live_authority_malformed_values_fail_closed(authority):
    assert not entry_safety.submission_authority_valid(authority).allowed
...
def test_retired_lift_can_never_remove_kill_switch():
    root = Path(__file__).resolve().parents[1]
    assert not (root / 'ops' / 'lift_killswitch_retired.sh').exists()
...
def test_every_automated_entry_placeorder_site_uses_live_authority_guard():
    root = Path(__file__).resolve().parents[1]
    for rel in ("daily_recommend.py", "exitmgr/trader.py"):
        lines = (root / rel).read_text().splitlines()
        sites = [idx for idx, line in enumerate(lines) if ".placeOrder(" in line]
        assert len(sites) == 2, f"unexpected automated entry money-boundary count in {rel}"
        for idx in sites:
            context = "\n".join(lines[max(0, idx - 5):idx + 6])
            assert "place_with_live_authority(" in context, (
                f"unguarded automated entry placeOrder in {rel}:{idx + 1}"
            )
