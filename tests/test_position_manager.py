...
def test_assess_parses_and_filters(monkeypatch):
    ...
    assert out[111]["action"] == position_manager.OUT_OF_REMIT
    assert out[111]["proposed_action"] == "arm_trail"
    assert out[111]["trail_activation_gain_pct"] == 40
    assert out[222]["proposed_action"] == "bogus"
    assert out[333]["action"] == "cut" and "proposed_action" not in out[333]
...
def test_assess_empty_on_error(monkeypatch):
    ...
    out, meta = position_manager.assess_positions(
        "ep", "m", [{"con_id": 1}], return_meta=True)
    assert out == {}
    assert meta["attempt_ok"] is False
    assert meta["failure_reason"].startswith("RuntimeError: down")
...
def test_reason_not_truncated_at_200(monkeypatch):
    ...
    monkeypatch.setattr(position_manager, "_post_json", lambda *a, **k: raw)
    out = position_manager.assess_positions("ep", "m", [{"con_id": 111}])
    assert out[111]["reason"] == long_reason
...
def test_reason_bounded_at_8k(monkeypatch):
    ...
    monkeypatch.setattr(position_manager, "_post_json", lambda *a, **k: raw)
    out = position_manager.assess_positions("ep", "m", [{"con_id": 111}])
    assert len(out[111]["reason"]) == 8000
...
def test_return_meta_threads_raw_and_default_unchanged(monkeypatch):
    ...
    assert meta["raw"] == raw
    assert meta["attempt_ok"] is True and meta["failure_reason"] is None
...
def test_return_meta_raw_present_even_on_unparseable(monkeypatch):
    ...
    assert meta["attempt_ok"] is False
    assert meta["failure_reason"] == "invalid_response_json"


def test_return_meta_empty_when_no_model_or_positions():
    """Public API contract; production-derived narrative omitted."""
    for out, meta in (
            position_manager.assess_positions("ep", "", [{"con_id": 1}], return_meta=True),
            position_manager.assess_positions("ep", "m", [], return_meta=True)):
        assert out == {}
        assert meta["raw"] is None and meta["model_identity_source"] is None
        assert meta["attempt_ok"] is None
        assert meta["failure_reason"].startswith("not_attempted:")
