...
def _write_dataset(tmpdir):
    ...
    no_trade = {
        ...
        "_dedup_key": "no_trade:fixture-exact",
...
def _exclude_fixture_no_trade(ds):
    rows = di.read_source_rows(ds)
    _line, row, digest = next(item for item in rows
                              if item[1] and item[1].get("kind") == "no_trade")
    return di.append_training_exclusion(
        ds, target_line_sha256=digest, target_dedup_key=row["_dedup_key"],
        target_kind=row["kind"], target_source=row["source"], target_ts=row["ts"],
        reason_code="NONEMPTY_STAGE_A_ZERO_STAGE_B_CANDIDATES_MISLABELED_EMPTY_SLATE",
        evidence_line_sha256=["b" * 64], created_at="2026-08-25T16:00:00-07:00")


@pytest.mark.parametrize("fmt", ["jsonl-flat", "chat"])
def test_exact_historical_training_exclusion_is_receipted_in_both_formats(tmp_path, fmt):
    ds = _write_dataset(str(tmp_path))
    receipt = _exclude_fixture_no_trade(ds)
    out = str(tmp_path / f"excluded-{fmt}.jsonl")
    counts = etd.export(str(tmp_path), out, fmt, include_open=False, min_realized=None)
    assert counts["training_excluded"] == 1
    assert counts["emitted"] == 3
    dropped = [json.loads(line) for line in open(out + ".dropped.jsonl")]
    historical = [row for row in dropped if row["reason"] == "historical_training_exclusion"]
    assert len(historical) == 1
    assert historical[0]["source_line_sha256"] == receipt["record"]["target_line_sha256"]
    manifest = json.load(open(out + ".manifest.json"))
    assert manifest["schema"] == "trade-training-export.v2"
    assert manifest["training_exclusion_records"] == 1
    assert manifest["training_excluded_records"] == 1
    assert manifest["training_exclusion_ledger_sha256"]
    assert manifest["source_dataset_sha256"]


def test_training_exclusion_follows_exact_row_across_reordering(tmp_path):
    ds = _write_dataset(str(tmp_path))
    _exclude_fixture_no_trade(ds)
    path = tmp_path / "trade_dataset.jsonl"
    lines = path.read_bytes().splitlines(keepends=True)
    path.write_bytes(b"".join(reversed(lines)))
    out = str(tmp_path / "reordered.jsonl")
    counts = etd.export(str(tmp_path), out, "jsonl-flat", False, None)
    assert counts["training_excluded"] == 1 and counts["emitted"] == 3


def test_source_drift_aborts_before_existing_output_is_rotated(tmp_path):
    ds = _write_dataset(str(tmp_path))
    _exclude_fixture_no_trade(ds)
    out = tmp_path / "existing.jsonl"
    out.write_text("preserve-me\n")
    path = tmp_path / "trade_dataset.jsonl"
    path.write_text(path.read_text().replace("Nothing clears the bar today.", "MUTATED"))
    with pytest.raises(di.DatasetIntegrityError):
        etd.export(str(tmp_path), str(out), "jsonl-flat", False, None)
    assert out.read_text() == "preserve-me\n"
    assert not list(tmp_path.glob("existing.jsonl.LEGACY_NOT_FOR_TRAINING.*"))


def test_export_refuses_when_integrity_module_is_unavailable(tmp_path, monkeypatch):
    _write_dataset(str(tmp_path))
    monkeypatch.setattr(etd, "_di", None)
    with pytest.raises(RuntimeError, match="integrity module unavailable"):
        etd.export(str(tmp_path), str(tmp_path / "never.jsonl"), "jsonl-flat", False, None)
