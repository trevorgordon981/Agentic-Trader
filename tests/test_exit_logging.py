...
def _mgr(tmp_path, journal_lines, audit_lines=None, runtime_identity=None):
    cfg = Config()
    cfg.dry_run = True
    cfg.loop_mode = False
    ...
    return ExitManager(cfg, runtime_identity=runtime_identity), cfg
...
def test_log_exit_spread_records_legs_and_loss(tmp_path):
    je = {"ts": "2026-06-18T16:09:33+00:00", "contract_id": 222, "symbol": "MUTX",
    ...
    mgr._log_exit(222, "MUTX", trig, exit_price_per_share=1.48, quantity=1,
                  reason=mgr._exit_reason(trig))
    r = _read_exits(cfg)[0]
...
def test_recovered_legacy_close_stays_explicitly_unknown_not_current(tmp_path):
    identity_b = RuntimeIdentity("b" * 40, "c" * 64)
    je = {"contract_id": 445, "symbol": "IWM", "quantity": 1, "debit": 100.0}
    mgr, cfg = _mgr(tmp_path, [je], runtime_identity=identity_b)
    ok = mgr._log_exit(
        445, "IWM", types.SimpleNamespace(trigger_type="stop"),
        exit_price_per_share=0.8, quantity=1, reason="stop",
        extra={"code_version": None, "policy_version": None})
    assert ok
    row = _read_exits(cfg)[0]
    assert row["code_version"] == row["policy_version"] == ""


def test_close_created_under_a_then_finalized_under_b_records_a(tmp_path):
    identity_a = RuntimeIdentity("a" * 40, "d" * 64)
    identity_b = RuntimeIdentity("b" * 40, "e" * 64)
    je = {"contract_id": 446, "symbol": "SPY", "quantity": 1, "debit": 100.0}
    mgr, cfg = _mgr(tmp_path, [je], runtime_identity=identity_b)
    assert mgr._log_exit(
        446, "SPY", types.SimpleNamespace(trigger_type="stop"),
        exit_price_per_share=0.8, quantity=1, reason="stop",
        extra=identity_a.as_dict())
    row = _read_exits(cfg)[0]
    assert row["code_version"] == identity_a.code_version
    assert row["policy_version"] == identity_a.policy_version


@pytest.mark.parametrize("extra", [
    {"code_version": "a" * 40, "policy_version": ""},
    {"code_version": "bad", "policy_version": "d" * 64},
])
def test_half_or_invalid_exit_authority_refuses_the_record(tmp_path, extra):
    je = {"contract_id": 447, "symbol": "SPY", "quantity": 1, "debit": 100.0}
    mgr, cfg = _mgr(tmp_path, [je])
    ok = mgr._log_exit(
        447, "SPY", types.SimpleNamespace(trigger_type="stop"),
        exit_price_per_share=0.8, quantity=1, reason="stop", extra=extra)
    assert not ok
    assert not (tmp_path / "exits.log").exists()
