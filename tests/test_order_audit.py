...
def _order_manager(tmp_path, trade=None, runtime_identity=None):
    ib_conn = MagicMock()
    if trade is None:
        trade = MagicMock()
    ...
    return OrderManager(ib_conn, sm, runtime_identity=runtime_identity), ib_conn, sm


def test_close_authority_is_stamped_durably_before_transmission(tmp_path):
    identity = RuntimeIdentity("a" * 40, "b" * 64)
    om, ib_conn, sm = _order_manager(tmp_path, runtime_identity=identity)

    async def _observe_durable_intent(_contract, _order):
        record = sm.state.get_in_flight(111)
        assert record is not None and record.placement_state == "intent"
        assert record.exit_context["code_version"] == "a" * 40
        assert record.exit_context["policy_version"] == "b" * 64
        trade = MagicMock()
        trade.order.orderId = 77
        trade.orderStatus.status = "Submitted"
        return trade

    ib_conn.place_order = AsyncMock(side_effect=_observe_durable_intent)
    result = asyncio.run(om.place_close_order(
        con_id=111, symbol="SPY", quantity=1, limit_price=2.50, entry_debit=120.0,
        live_open_orders={}, right="C", exit_context={"reason": "stop"}))
    assert result.success


def test_close_refuses_context_that_conflicts_with_frozen_authority(tmp_path):
    identity = RuntimeIdentity("a" * 40, "b" * 64)
    om, ib_conn, sm = _order_manager(tmp_path, runtime_identity=identity)
    result = asyncio.run(om.place_close_order(
        con_id=111, symbol="SPY", quantity=1, limit_price=2.50, entry_debit=120.0,
        live_open_orders={}, right="C",
        exit_context={"code_version": "c" * 40, "policy_version": "b" * 64}))
    assert not result.success and "conflicts with the frozen process identity" in result.message
...
def test_close_right_asserts_nothing_when_unresolvable(tmp_path):
    """Public API contract; production-derived narrative omitted."""
    om, ib_conn, _ = _order_manager(tmp_path)
    ib_conn.ib.portfolio = MagicMock(return_value=[])
    res = asyncio.run(om.place_close_order(
        con_id=111, symbol="SPY", quantity=1, limit_price=2.50, entry_debit=120.0,
        live_open_orders={}))
    assert res.success
    ib_conn.create_contract.assert_called_once_with(111, symbol="SPY", right="")


def test_close_right_is_not_silently_flipped_to_a_put(tmp_path):
    """Public API contract; production-derived narrative omitted."""
    om, ib_conn, _ = _order_manager(tmp_path)
    ib_conn.ib.portfolio = MagicMock(side_effect=RuntimeError("Not connected to IB"))
    res = asyncio.run(om.place_close_order(
        con_id=111, symbol="SPY", quantity=1, limit_price=2.50, entry_debit=120.0,
        live_open_orders={}))
    assert res.success
    _, kwargs = ib_conn.create_contract.call_args
    assert kwargs["right"] not in ("C", "P")
