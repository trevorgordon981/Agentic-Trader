...
# ---------------- risk / gross screen (debit = capital at risk, NOT strike notional)

def _screen_blocks(r, net_liq):
    """Mirror the entry risk screen in trader.py / daily_recommend.py: it measures the actual
    capital at risk (net debit = max loss) and blocks when it exceeds the 30x-NetLiq cap."""
    est_gross = r.limit * 100 * r.qty
    return net_liq > 0 and est_gross > 0.95 * 30 * net_liq


def test_cheap_debit_spread_passes_screen():
    # ORCL 162.5/160P debit spread @ $1.25 on a ~$629 account: max loss $125. Strike notional
    # would be ~$32k (the OLD bug); the real risk is the $125 debit, which must PASS.
    r = ResolvedOrder("ORCL", "P", "20260717", 162.5, 1, 1.25, _contract(111),
                      short_strike=160.0, short_contract=_contract(222))
    assert r.limit * 100 * r.qty == 125.0          # capital at risk = net debit
    assert not _screen_blocks(r, net_liq=629.0)     # 30x cap ~$18.9k -> passes


def test_cheap_single_leg_passes_screen():
    r = ResolvedOrder("SPY", "C", "20260620", 610.0, 1, 1.20, _contract(111))
    assert not _screen_blocks(r, net_liq=629.0)     # $120 risk << cap


def test_oversized_debit_still_rejected():
    # A genuinely oversized debit: $200/share net debit, 3 contracts = $60k max loss on a
    # ~$629 account -> far exceeds the 30x cap (~$18.9k) and MUST still be rejected.
    r = ResolvedOrder("NVDA", "C", "20260717", 200.0, 3, 200.0, _contract(111))
    assert r.limit * 100 * r.qty == 60000.0
    assert _screen_blocks(r, net_liq=629.0)
