...
def test_put_spread_shorts_lower_strike():
    ...
    assert pick_spread_short(cands, 290.0, 2.0, "P", 121.0) == (285.0, 0.80)


def test_none_when_no_usable_short():
    assert pick_spread_short([(305.0, 2.39)], 300.0, 2.40, "C", 121.0) is None
    assert pick_spread_short([(295.0, 3.0)], 300.0, 2.40, "C", 121.0) is None
...
def _trader(tmp_path):
    ibc = MagicMock()
    ibc.ib = MagicMock()
    _trade = ibc.ib.placeOrder.return_value
    _trade.orderStatus.status = "Filled"
    _trade.orderStatus.avgFillPrice = 1.20
    _trade.fills = []
    ...
    def _fill_completely(_contract, _order):
        _trade.orderStatus.filled = float(getattr(_order, "totalQuantity", 0) or 0)
        _trade.orderStatus.remaining = 0.0
        return _trade
...
def test_cheap_debit_spread_passes_screen():
    ...
    assert r.limit * 100 * r.qty == 125.0
    assert not _screen_blocks(r, net_liq=629.0)
...
def test_spread_close_places_atomic_bag(tmp_path):
    ...
    ib_conn.create_contract.assert_not_called()
    ib_conn.create_limit_order.assert_called_once_with("SELL", 1, 2.50)
    assert sm.state.get_in_flight(111) is not None
