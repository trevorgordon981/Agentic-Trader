...
tp, sl = C.clamp_tp_sl(100.0, 50.0, cons)
...
check("SYMF 14/25 lottery vertical REJECTED (IV path)", not ok, why)
...
check("far-OTM long strike (SYN1-style) REJECTED", not ok, why)
...
ok, why = C.check_budget(300.0, 30, 1000.0, [], cons)
check("oversized premium (30% > 15%) REJECTED", not ok, "; ".join(why))
ok, why = C.check_budget(140.0, 30, 1000.0, [], cons)
...
ok, why = C.check_budget(150.0, 10, 1000.0, [], cons)
